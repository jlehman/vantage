...
nonisolated enum CodexSettingsInstallerError: Error, Equatable, LocalizedError {
  ...
  var errorDescription: String? {
    switch self {
    case .codexUnavailable:
      "Codex must be installed and available in your login shell before Vantage can install hooks."
    case .enableHooksFailed(let details):
      details.isEmpty
        ? "Vantage could not enable the Codex hooks feature."
        : "Vantage could not enable the Codex hooks feature: \(details)"
    case .invalidEventHooks(let event):
      "Codex hooks use an unsupported shape for \(event)."
    case .invalidHooksObject:
      "Codex hooks use an unsupported shape."
    case .invalidJSON(let detail):
      "Codex hooks must be valid JSON before Vantage can install hooks (\(detail))."
    case .invalidRootObject:
      "Codex hooks must be a JSON object before Vantage can install hooks."
    }
  }
}
