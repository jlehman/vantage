...
/// `SUPACODE_SURFACE_ID` so it no-ops outside a Vantage surface.
///
...
public nonisolated enum AgentPresenceOSC {
  /// Env var present only on Vantage surfaces, so its presence is the
  /// no-op-outside-Vantage emit gate.
