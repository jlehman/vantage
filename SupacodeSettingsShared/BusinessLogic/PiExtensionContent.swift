/// Bundled TypeScript extension that Vantage installs into
/// `~/.pi/agent/extensions/vantage/index.ts` to report agent
/// lifecycle hooks back to the Vantage macOS app.
nonisolated enum PiExtensionContent {
  /// Directory name under `~/.pi/agent/extensions/`.
  static let extensionDirectoryName = "vantage"

  /// Marker comment used to identify Vantage-managed extensions.
  static let ownershipMarker = "/* vantage-managed-extension */"

  static let indexTs = """
    \(ownershipMarker)
    /**
     * Vantage + Pi integration extension.
     *
     * Reports agent lifecycle and notifications to Vantage by emitting OSC 3008
     * escape sequences to the controlling terminal. The sequences are inert in any
     * terminal that does not handle OSC 3008, and reach Vantage over SSH too (no
     * local socket needed), matching the Claude / Codex / Kiro hook integrations.
     *
     * Required env var (injected automatically by Vantage on every surface):
     *   SUPACODE_SURFACE_ID  present only on a Vantage surface; absence is the
     ...
     * from "not a Vantage surface" without spamming the log on every emit.
     */
      ...
      try {
        ...
        if (now - lastWarnedAt > WARN_INTERVAL_MS) {
          ...
          process.stderr.write(
            `vantage: OSC emit failed: code=${code} errno=${errno} message=${message}\\n`,
          );
        }
      }
    }
    ...
    export default function (pi: ExtensionAPI) {
      // Not running under Vantage, or not a Vantage surface: stay inert.
      ...
      pi.on("agent_end", (_event, ctx) => {
        ...
        // Vantage side (turn-level Stop equivalent).
        emitPresence("idle");
