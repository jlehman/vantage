...
nonisolated struct ClaudeSettingsInstaller {
  ...
  /// duplicates left by older Vantage versions.
...
nonisolated enum ClaudeSettingsInstallerError: Error, Equatable, LocalizedError {
  ...
  var errorDescription: String? {
    ...
    case .invalidJSON(let detail):
      "Claude settings must be valid JSON before Vantage can install hooks (\(detail))."
    case .invalidRootObject:
      "Claude settings must be a JSON object before Vantage can install hooks."
    }
  }
}
