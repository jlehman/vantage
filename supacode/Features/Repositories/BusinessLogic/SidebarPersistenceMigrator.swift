...
///
/// Writes to:
/// - `~/.vantage/sidebar.json` via the shared
...
enum SidebarPersistenceMigrator {
  ...
  static func migrateIfNeeded(
    ...
    // base, default `~/.vantage/repos/<name>/` convention) so
  ...
  ///   order) per-repo `vantage.json` override → global
  ///   `defaultWorktreeBaseDirectoryPath` override → default
  ///   `~/.vantage/repos/<lastPathComponent>/` convention.
  ...
  /// corrupt `vantage.json` files are skipped silently; the
  ...
  private static func rootCandidates(
    ...
    for owningRoot in legacyRoots {
      ...
      //    `~/.vantage/repos/<name>/` convention when no override
      //    is configured.
  ...
  /// Synchronously reads `<root>/vantage.json` via the injected
  ...
  /// `~/.vantage/repos/<name>/` still resolves to the settings
